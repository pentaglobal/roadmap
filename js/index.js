...
$(function() {
  ...
  var planRoad = {
    tpl: {
      ...
      quarterly: [
        '<div class="quarterly">',
        '  <span class="text">',
        '    <span class="quarterly-name">{name}{desc}</span>',
        "  </span>",
        "</div>"
      ].join(""),
      star:
        '<span class="quarterly-icon animation"><img src="./img/star1.png"/></span>',
      github:
        '<span class="quarterly-icon animation1"><img src="./img/star2.png"/></span>',
      round:
        '<span class="quarterly-icon"><span class="icon-round"><i class="icon-round-inner"></i></span></span>'
    },
    init: function() {
    ...
    renderPercent: function($wrap, plan) {
      if (plan.percent) {
        ...
        $percent.css({
          clip:
            "rect(0px, " +
            $percent.outerWidth() * plan.percent / 100 +
            "px, 120px, 0px)"
        });
      }
    },
      ...
      for (var i = 0; i < children.length; i++) {
        ...
        if (item && item.length) {
          for (var k = 0; k < item.length; k++) {
            item[k].finishCls = item[k].isFinish ? "finish" : "";
  ...
  var yearAxis = {
    tpl: [
      '<div class="axis-item" style="top:{top}px">',
      '  <span class="star"><img src="./img/star1.png"></span>',
